...
2. Create a virtual environment and install dependencies. The project is installed in editable mode together with the `dev` dependency group.
   ...
   ```
3. Install [poethepoet](https://github.com/nat-n/poethepoet) 0.33 or later, a task runner. It detects the uv project and runs each task via `uv run`. Ensure that the following tasks work.
   ```bash
   poe lint
   poe type-check
   poe test
